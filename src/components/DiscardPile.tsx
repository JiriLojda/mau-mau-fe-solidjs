...
export const DiscardPile = (props: Props) => {
  const [cardRotations, setCardRotations] = createSignal<ReadonlyArray<number>>(
    props.cards.map(() => randomInt(0, 359)),
  );

  createEffect(() => {
    if (cardRotations().length !== props.cards.length) {
      const remainingRotations = cardRotations().slice(0, props.cards.length);
      const newRotations = props.cards.slice(cardRotations().length).map(() => randomInt(0, 359));
        ...
        {(pair, index) => (
          <CardComponent
            style={{
              ...
              right: `${cardHeight / 2}px`,
              transform: `rotate(${pair()[1]}deg)`,
            }}
